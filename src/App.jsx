  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { l: "Active", v: active.length, c: "text-emerald-400" },
          { l: "Blocked", v: blocked.length, c: "text-red-400" },
          { l: "In Progress", v: inProgress.length, c: "text-blue-400" },
          {
            l: "Stale (7d+)",
            v: stale.length + ghStale.filter((p) => !stale.includes(p)).length,
            c: "text-amber-400",
          },
        ].map((x) => (
          <div key={x.l} className="rounded-xl bg-slate-800/60 border border-slate-700/50 p-4">
      ...
      {nextSteps.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-slate-200 mb-3">Next Steps</h2>
          <div className="space-y-2">
            {nextSteps.map((p) => (
              <button
                key={p.id}
      ...
      {inProgress.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-blue-400 mb-3">Currently In Progress</h2>
          <div className="space-y-2">
            {inProgress.map((t) => (
              <button
                key={t.id}
    ...
    <div className="group">
      ...
      {editing ? (
        ...
        >
          ...
          <span className="ml-auto opacity-0 group-hover:opacity-100 text-slate-500 transition-opacity shrink-0 mt-0.5">
            <IconEdit size={14} />
          </span>
        </button>
      )}
